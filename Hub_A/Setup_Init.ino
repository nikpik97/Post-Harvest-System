...
void setupBluetooth()
{
  ...
  BTSerial.print(F("AT+NAMEHub"));
  BTSerial.readLine();

  BTSerial.print(F("AT+PIO11"));
  BTSerial.readLine();
  
  // SETS UUID FOR APP
  #if APP_DEBUG
  BTSerial.print("AT+UUID0xECE0");
  BTSerial.readLine();
  #else
  BTSerial.print("AT+UUID0xFFE0");
  BTSerial.readLine();
  #endif
...
void setupInitial()
{
  ...
  // Check if there has been an issue with getting HubID
  if((HubID[0] == '0') && (HubID[1] == '0') && (HubID[2] == '0') && (HubID[3] == '0'))
    errorBlink();
...
void printSetup()
{
  ...
  Serial.print(F("Logging Freq: "));
  Serial.println(LoggingFreq);
